...
public class FirstPersonAIO : MonoBehaviour
{
    ...
    public FootStepScript footStep;
    public AudioClip[] woodFootSteps;
    Vector3 current_pos;
    Vector3 last_pos;
    public float speed2 = 0f;
    public float gravity2 = -200f;
    Vector3 velocity;

    Vector3 standGrounded;
    Vector3 crouchGrounded;
    Transform curGrounded;

    LayerMask ventLayerMask;
    GameObject terrain;
...
public class BETA_SETTINGS{
    ...
    private PlayerEnemyInteraction bossDetection;
    private int maxVelocityChange = 4;
    ...
    {
        IsGrounded = true;
        footStep = GetComponent<FootStepScript>();
        terrain = footStep.terrain;
        standGrounded = footStep.standingFoot.position;
        crouchGrounded = footStep.standingFoot.position;
    ...
    private void Start()
    {
        ...
        terrain = GetComponent<FootStepScript>().terrain;
        ventLayerMask = footStep.ventLayerMask;
        onStair = false;
        ...
        bossDetection = GetComponent<PlayerEnemyInteraction>();
    ...
    private void Update()
    {
        ...
        if (useStamina)
        {
            isSprinting = Input.GetKey(sprintKey) && !isCrouching && staminaInternal > 0 && Mathf.Abs(fps_Rigidbody.velocity.x) > 0.01f;
            if (isSprinting)
            {
        ...
        if (_crouchModifiers.useCrouch && playerCanMove)
        {
            ...
            else if (!above)
            {
                ...
                capsule.height = Mathf.MoveTowards(capsule.height, 2, 5 * Time.deltaTime);
                walkSpeedInternal = walkSpeed;
                sprintSpeedInternal = sprintSpeed;
                jumpPowerInternal = jumpPower;
            }
        ...
        /* current_pos = transform.position;
         speed2 = (current_pos - last_pos).magnitude / Time.deltaTime;
         velocity2 = (current_pos - last_pos) / Time.deltaTime;
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            //dMove = (myDirection * inputXY.y * speed + transform.right * inputXY.x * walkSpeedInternal) /* + Vector3.up * fps_Rigidbody.velocity.y*/;
            dMove = myDirection;
        ...
        /*  // fps_Rigidbody.AddForce(Physics.gravity * (advanced.gravityMultiplier - 1));
          ...
          {
              fps_Rigidbody.AddForce(Vector3.down * 800f, ForceMode.Force);
          }
          else
          {
              fps_Rigidbody.AddForce(Vector3.down * 800f, ForceMode.Force);
    ...
    void HandleGravity()
    {
        float currentVerticalSpeed = fps_Rigidbody.velocity.y;
